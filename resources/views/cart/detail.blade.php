...
{{-- @endphp)--}}
                        @if($carts && count($carts) > 0)
                            @foreach($carts as $cart)
                                ...
                                <div class="order-box shadow-sm transition px-4 py-1 mt-4 rounded-2 p-4 position-relative">
                                    <form action="{{ route('cart.delete', $cart['product_id']) }}" method="POST" id="delete-form-{{ $cart['product_id'] }}">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-danger rounded-circle fs-12 position-absolute top-0 start-100 translate-middle p-0 px-1">
